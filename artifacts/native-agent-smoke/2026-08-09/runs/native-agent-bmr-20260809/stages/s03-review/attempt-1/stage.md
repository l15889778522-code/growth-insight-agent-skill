...
| role | `growth-review` |
| attempt | `1` |
| stage_status | `PASS_WITH_RISKS` |
| agent_contract_version | `1.2` |

## 摘要

The candidate metric contract is consistent with the approved Business scope and available schema fields. Operational computation remains intentionally unresolved pending approval of event mappings, window boundaries, cohort-maturity rules, and plan assignment timing.
...
```json
{
  ...
  "optional_improvements": [
    "Approve the first-activation event or event-sequence mapping.",
    "Approve the core_action event_name mapping.",
    "Approve the first-activation qualification window.",
    "Approve the day-seven observation-window boundaries and cohort-maturity rule.",
    "Approve the timestamp used to assign plan."
  ],
  "required_fixes": [],
  "rollback_stage": null
}
...
- recommended_next_stage: `END`
- required_next_inputs: `0` item(s)
